"""
Streamlit RAG Application with LangGraph
A conversational RAG system with chat interface
"""
...
def get_all_documents():
    """Get all .txt files from documents folder"""
...
def get_vectorstore():
    ...
    if os.path.exists(VECTOR_STORE_PATH):
        ...
        if len(doc_files) == 0:
            st.error("❌ No documents found! Please upload .txt files using the sidebar.")
            return None
        ...
        # Create documents with metadata (filename as key)
        all_docs = []
...
def main():
    st.set_page_config(
        page_title="RAG Chat with LangGraph",
        page_icon="🤖",
        layout="wide"
    )

    st.title("🤖 RAG Chat Application with LangGraph")
    ...
    with st.sidebar:
        ...
        st.markdown("""
        1. Upload .txt files with your documents
