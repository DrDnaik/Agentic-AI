...
def main():
    ...
    st.markdown("<h1 style='color:blue; text-align:center;'>Sentiment Analyzer</h1>", unsafe_allow_html=True)

    st.markdown('##### Generate  responses according to sentiments using AI Powered Langgraph Agents')

    col1, col2 = st.columns([1, 1])

    with col1:
        # st.subheader('Input  from user')
        st.markdown("<h1 style='color:red; text-align:center;'>Input from User</h1>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown("<h1 style='color:green; text-align:center;'>Agent Response</h1>", unsafe_allow_html=True)
        if st.button('Generate Response'):
            if user_input:
                with st.spinner('AI sentiment Agent is getting generated ...'):
                    result = process_user_input(user_input)

                st.success('Sentiment generated successfully')

                st.subheader('AI Generated response')
                st.text_area("Your AI Generated response",
